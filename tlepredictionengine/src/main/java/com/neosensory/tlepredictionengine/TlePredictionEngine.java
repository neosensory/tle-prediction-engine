...
/**
 * This class provides methods useful for obtaining latitude/longitude/altitude coordinates for a
 * given satellite based on its two line element (TLE) set and time
 *
...
public class TlePredictionEngine {
   ...
   * @param useGeodetic boolean. If true, return coordinates in geodetic latitude (degrees),
   *     longitude (degrees), and altitude (km). If false, the method will return the TEME
   *     coordinates (x,y,z) in km.
   * @return coordinates of the satellite at the current time in either geodetic or TEME
   */
  public static double[] getSatellitePosition(
      String tleLine1, String tleLine2, Boolean useGeodetic) {
    ...
    if (!useGeodetic) {
      ...
      double[] latLonAlt =
          TemeGeodeticConverter.getLatLonAlt(positions[0], positions[1], positions[2], currentTime);
      return latLonAlt;
    }
  }
   ...
   */
  public static double[] getSatellitePosition(
      String tleLine1, String tleLine2, Boolean useGeodetic, Date date) {
    TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
    Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
    cal.setTime(date);
    Date currentTime = cal.getTime();
