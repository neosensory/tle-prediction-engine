...
/**
 * This class provides sgp4 procedures for analytical propagation * of a satellite. the code was
 * originally released in the 1980 and 1986 * spacetrack papers. a detailed discussion of the theory
 * and history * may be found in the 2006 aiaa paper by vallado, crawford, hujsak, * and kelso.
 *
 * @author David Valado
 */
public class Sgp4 {
  ...
  public static void dpper(
    ...
    if (init == 'n') {
      ...
      if (rec.inclp >= 0.2) {
        ...
        if ((rec.nodep < 0.0) && (opsmode == 'a')) rec.nodep = rec.nodep + twopi;
        ...
        if (Math.abs(xnoh - rec.nodep) > pi)
          if (rec.nodep < xnoh) rec.nodep = rec.nodep + twopi;
          else rec.nodep = rec.nodep - twopi;
        rec.mp = rec.mp + pl;
  ...
  *    vallado, crawford, hujsak, kelso  2006
  ...
  public static void dsinit(
      ...
      double tc, double xpidot, ElsetRec rec) {
    /* --------------------- local variables ------------------------ */
    ...
    if ((rec.inclm < 5.2359877e-2) || (rec.inclm > pi - 5.2359877e-2)) shs = 0.0;
    if (rec.sinim != 0.0) shs = shs / rec.sinim;
    ...
    // sgp4fix for 180 deg incl
    if ((rec.inclm < 5.2359877e-2) || (rec.inclm > pi - 5.2359877e-2)) shll = 0.0;
    rec.domdt = sgs + sghl;
    rec.dnodt = shs;
    if (rec.sinim != 0.0) {
    ...
    if (rec.irez != 0) {
      ...
      if (rec.irez == 2) {
        ...
        if (rec.em <= 0.65) {
          ...
          else g520 = 1464.74 - 4664.75 * rec.em + 3763.64 * rec.emsq;
        }
        if (rec.em < 0.7) {
  ...
  *    vallado, crawford, hujsak, kelso  2006
  ...
  public static void dspace(double tc, ElsetRec rec) {
    ...
    if (rec.irez != 0) {
      ...
      if ((rec.atime == 0.0)
          || (rec.t * rec.atime <= 0.0)
          || (Math.abs(rec.t) < Math.abs(rec.atime))) {
        rec.atime = 0.0;
        rec.xni = rec.no_unkozai;
        rec.xli = rec.xlamo;
      }
      // sgp4fix move check outside loop
      if (rec.t > 0.0) delt = stepp;
      else delt = stepn;
      ...
      while (iretn == 381) {
        ...
        if (rec.irez != 2) {
          ...
          xndt =
              rec.d2201 * Math.sin(x2omi + rec.xli - g22)
          ...
          xldot = rec.xni + rec.xfact;
          xnddt =
              rec.d2201 * Math.cos(x2omi + rec.xli - g22)
                  ...
                  + 2.0
                      * (rec.d4410 * Math.cos(x2omi + x2li - g44)
  ...
  *    vallado, crawford, hujsak, kelso  2006
  ...
  public static boolean sgp4init(char opsmode, ElsetRec satrec) {
    /* --------------------- local variables ------------------------ */
    ...
    if ((satrec.omeosq >= 0.0) || (satrec.no_unkozai >= 0.0)) {
      satrec.isimp = 0;
      if (satrec.rp < (220.0 / satrec.radiusearthkm + 1.0)) satrec.isimp = 1;
      sfour = ss;
      qzms24 = qzms2t;
      ...
      if (Math.abs(satrec.cosio + 1.0) > 1.5e-12)
        satrec.xlcof =
            -0.25 * satrec.j3oj2 * satrec.sinio * (3.0 + 5.0 * satrec.cosio) / (1.0 + satrec.cosio);
      else satrec.xlcof = -0.25 * satrec.j3oj2 * satrec.sinio * (3.0 + 5.0 * satrec.cosio) / temp4;
  ...
  *    vallado, crawford, hujsak, kelso  2006
  ...
  public static boolean sgp4(ElsetRec satrec, double tsince, double r[], double v[]) {
    ...
    if (satrec.em < 1.0e-6) satrec.em = 1.0e-6;
    ...
    if (satrec.method == 'd') {
      ...
      else satrec.xlcof = -0.25 * satrec.j3oj2 * sinip * (3.0 + 5.0 * cosip) / temp4;
    }
